    ...
    const GroupList = () => (
        <div className="flex flex-col gap-2">
            {groupLists.map((group) => (
                <div
                    key={group.id}
                    className="bg-white border border-[#ede8f5] rounded-2xl px-3 py-2.5 flex items-center gap-3 cursor-pointer hover:border-[#c4b8f0] transition-colors shadow-sm"
                    onClick={() => router.push(`/groups/${group.id}`)}
                >
                    <div className="w-9 h-9 rounded-xl bg-available-me flex items-center justify-center text-base shrink-0 overflow-hidden">
                        {group.imageUrl ? (
                            <img src={group.imageUrl} className="w-full h-full object-cover" />
                        ) : '👥'}
                    </div>
                    <div className="flex-1">
                        <p className="text-sm font-medium text-text-dark">{group.name}</p>
                    </div>
                    <button
                        onClick={(e) => { e.stopPropagation(); handleCopy(group.inviteCode) }}
                        className="w-7 h-7 bg-input-bg border border-[#e0d8f8] rounded-lg flex items-center justify-center text-xs shrink-0"
                    >
                        🔗
                    </button>
                </div>
                        ...
                        <form onSubmit={handleCreate} className="flex flex-col gap-3">
                            ...
                            <div className="flex gap-2 mt-2">
                                <button
                                    type="button"
                                    onClick={() => setShowCreate(false)}
                                    className="flex-1 bg-white text-primary border border-[#c4b8f0] py-3 rounded-xl text-sm font-medium"
                                >
                                    ยกเลิก
                                </button>
                                <button type="submit" className="flex-1 bg-primary text-white py-3 rounded-xl text-sm font-medium">
                                    สร้างกลุ่ม
                                </button>
                            </div>
                        </form>
            ...
            <div className="md:hidden flex flex-col min-h-screen">
                <div className="bg-linear-to-r from-[#e8d5f5] to-[#ffd6e7] px-4 py-5 flex items-center justify-between">
                    ...
                    <button
                        ...
                        className="w-8 h-8 rounded-full bg-white border border-card-border flex items-center justify-center text-xs font-medium text-primary"
                    >
                        {avatarLetter}
                    </button>
                </div>
