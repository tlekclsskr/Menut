...
export default function GroupsPage() {
    ...
    useEffect(() => {
        Promise.all([fetchGroups(), fetchProfile()])
            .finally(() => setIsDataReady(true))
    }, [])

    if (!isReady || !isDataReady) return <LoadingSpinner />
                        ...
                        <form onSubmit={handleCreate} className="flex flex-col gap-3">
                            ...
                            <div className="flex gap-2 mt-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={isCreating}
                                    className={`flex-1 py-3 rounded-xl text-sm ${btnPrimary}`}
                                >
                                    {isCreating ? (
                                        <div className="flex items-center justify-center gap-2">
                                            <ButtonSpinner />
                                            {file ? 'กำลังอัปโหลด...' : 'กำลังสร้าง...'}
                                        </div>
                                    ) : 'สร้างกลุ่ม'}
                                </button>
                            </div>
